...
@time_function
def plot_schedule(schedule, option_number, combined_score, days_score, gap_score):
    ...
    plt.title(f'Schedule Option {option_number} - Combined Score: {combined_score}, Days Score: {days_score}, Gap Score: {gap_score}', zorder=10)
...
def plot_schedules(combinations):
    with PdfPages('schedules.pdf') as pdf:
        for i, (combination, combined_score, days_score, gap_score) in enumerate(combinations[:50], start=1):
            fig = plot_schedule(combination, i, combined_score, days_score, gap_score)
            pdf.savefig(fig)
            plt.close(fig)
